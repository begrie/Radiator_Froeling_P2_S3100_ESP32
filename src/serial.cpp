...
namespace radiator
{
    // #define SERIAL_TO_RADIATOR_RX 16_PIN // ESP32-standard: 16
    // #define SERIAL_TO_RADIATOR_TX 17_PIN // ESP32-standard: 17
    ...
    SerialPort::SerialPort(std::string_view devicename)
    {
        ...
        Serial_to_Radiator->begin(SERIAL_TO_RADIATOR_BAUD, SERIAL_8N1, SERIAL_TO_RADIATOR_RX_PIN, SERIAL_TO_RADIATOR_TX_PIN);

        std::cerr << millis() << " ms: Opened serial port " << devicename << " with " << SERIAL_TO_RADIATOR_BAUD << " baud and SERIAL_8N1 for connection to radiator Froeling P2/S3100 -> wait for answer ..." << std::endl;
    }
