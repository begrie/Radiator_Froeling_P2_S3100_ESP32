...
SemaphoreHandle_t radiator::ExternalSensors::semaphoreAcCurrentSensor = NULL; // Definition für AC-Strom Semaphor
...
bool radiator::ExternalSensors::initExternalSensors()
{
    ...
    semaphoreAcCurrentSensor = xSemaphoreCreateMutex();
...
{
    static int16_t temp = 0, humidity = 0;

#if WITH_DHT_SENSOR
    if (dhtGPIO != 0) // GPIO defined for functionality enabling?
    {
        if (xSemaphoreTake(semaphoreExternalSensors, pdMS_TO_TICKS(10)) == pdTRUE)
        {
            temp = lastRoomTemperature;
            humidity = lastRoomHumidity;
            xSemaphoreGive(semaphoreExternalSensors);
        }
        else
        {
            // use old/previous values
            //  temp = -999; // Fehlerwert
            //  humidity = -999; // Fehlerwert
            RADIATOR_LOG_ERROR(getMillisAndTime() << "getSensorValues: Fehler beim Nehmen des Semaphores!" << std::endl;)
        }
    }
#endif // WITH_DHT_SENSOR
    ...
    return "[Heizungsraum-Temp] = [" +
           std::to_string(temp) + "°] \n" +
           "[Heizungsraum-Feuchtigkeit] = [" + std::to_string(humidity) + "%] \n" +
...
std::string radiator::ExternalSensors::getSensorValuesForJSON()
{
    ...
    if (dhtGPIO != 0) // GPIO defined for functionality enabling?
    {
        ...
        else
        {
            ...
            // temp = -999;     // Fehlerwert
            // humidity = -999; // Fehlerwert
            RADIATOR_LOG_ERROR(getMillisAndTime() << "getSensorValuesForJSON: Fehler beim Nehmen des Semaphores!" << std::endl;)
        }
    }
#endif // WITH_DHT_SENSOR

    return "\"Heizungsraum-Temp\": \"" + std::to_string(temp) + "°\"," +
           "\"Heizungsraum-Feuchtigkeit\": \"" + std::to_string(humidity) + "%\"," +
...
std::string radiator::ExternalSensors::getSensorValueDataForCSV()
{
    ...
    return std::to_string(temp) + "°" DELIMITER_FOR_CSV_FILE +
           std::to_string(humidity) + "%" DELIMITER_FOR_CSV_FILE +
...
void radiator::ExternalSensors::xTaskExternalSensors(void *parameter)
{
    ...
    while (true) // the xTask runs "endless"
    {
        ...
        {
            // Semaphore mit Timeout von z.B. 10ms nehmen
            if (xSemaphoreTake(semaphoreExternalSensors, pdMS_TO_TICKS(10)) == pdTRUE)
            {
            ...
            {
                RADIATOR_LOG_ERROR(getMillisAndTime() << "xTaskExternalSensors: Fehler beim Nehmen des Semaphores!" << std::endl;)
                // Fehler behandeln: z.B. DHT-Messung überspringen oder alten Wert verwenden
            }
        }
#endif // WITH_DHT_SENSOR
...
void IRAM_ATTR radiator::ExternalSensors::tickerCallbackForReadAndAverageAcCurrentSensor()
{
    ...
    if (millis() - timestamp >= 20) // 20ms -> one period for 50Hz mains frequency
    {
        ...
        if (periodCounter >= periodsForAveraging)
        {
            ...
            if (xSemaphoreTake(semaphoreAcCurrentSensor, 0) == pdTRUE) // im ticker context nicht warten!
            {
                // desired value is half of the peak to peak value
                acCurrentAnalogReadAmplitudeMillivolt = (maxSum - minSum) / (periodsForAveraging * 2);
                xSemaphoreGive(semaphoreAcCurrentSensor);
            }
            else
            {
                // Optional: Fehler zählen oder ignorieren
            }
...
{
    static double rmsAmpere = 0.0;
    if (xSemaphoreTake(semaphoreAcCurrentSensor, pdMS_TO_TICKS(10)) == pdTRUE)
    {
        auto peakCurrent = (double)(acCurrentAnalogReadAmplitudeMillivolt) * (double)AC_CURRENT_SENSOR_SCALE_AMPERE_PER_MILLIVOLT;
        rmsAmpere = peakCurrent * 0.70710678118; // calculate rms from amplitude
        xSemaphoreGive(semaphoreAcCurrentSensor);
    }
    else
    {
        // use old value from static variable
        RADIATOR_LOG_ERROR(getMillisAndTime() << "getAcCurrentAmpereRMS: Fehler beim Nehmen des Semaphores!" << std::endl;)
