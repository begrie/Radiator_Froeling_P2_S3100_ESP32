            ...
            for (int i = 0; i < command[2] / 2; ++i)
            {
                ...
                case PNT_VALUE:
                {
                    ...
                    if (dataformatIter != this->dataformats.end())
                    {
                        ...
                        // statischer Stringstream zur Heap-Fragmentierungsvermeidung
                        this->m_stringValueStream.str("");
                        this->m_stringValueStream.clear();
                        this->m_stringValueStream << std::fixed << std::setprecision(dataformat.decimals) << dblValue;
                        this->m_stringValueStream << dataformat.unit;
    ...
    void Surveillance::parseEndOfBlock(const uint8_t *command)
    {
        ...
        if (debug_level)
        {
            ...
            case 'U':
                ...
                this->m_allLastErrorsStream.str("");
                this->m_allLastErrorsStream.clear();
                this->m_allLastErrorsStream << "Last errors:" << std::endl;
                ...
                // RADIATOR_LOG_INFO( m_allLastErrorsStream.str();

                //"mis-use" handleError: send to output handler for information of restart, file saving etc.
                this->handler.handleError(*this,
                                          2100, 01, 01, // filename is derived from this date -> should be the "last date" not to be deleted by automatic filesystem cleaning of the oldest files
                                          0, 0, 0,
                                          this->m_allLastErrorsStream.str());
